...
class HideMyWP {
    ...
    function admin_notices(){
        ...
        if (stristr($_SERVER['SERVER_SOFTWARE'], 'nginx') || stristr($_SERVER['SERVER_SOFTWARE'], 'wpengine') )
            echo '<div class="error"><p>' . __('It seems you use Nginx web server! To configure Hide My WP go to start tab, click on Nginx Configuration and follow documentation!', self::slug ) . '</p></div>';
            
        if (stristr($_SERVER['SERVER_SOFTWARE'], 'iis') || stristr($_SERVER['SERVER_SOFTWARE'], 'Windows') )
            echo '<div class="error"><p>' . __('It seems you use Windows web server! Hide My WP doesn\'t offically support it but you may still make it work! Read the <a href="http://codecanyon.net/item/hide-my-wp-no-one-can-know-you-use-wordpress/4177158/faqs/16072">FAQ</a>', self::slug ) . '</p></div>';
   ...
   /**
    ...
    function pp_settings_api_filter($post){
        ...
        if (isset ($_POST[$this->settings_menu['name']]['li']) && (strlen($_POST[$this->settings_menu['name']]['li']) < 35 || strlen($_POST[$this->settings_menu['name']]['li']) > 40))
    ...
    function global_html_filter($buffer){
        ...
        if ($this->opt('remove_html_comments'))  {
            if ($this->opt('remove_html_comments')=='quick') {
...
;
        ...
        if ($this->opt('new_plugin_path') && $this->is_permalink()){    
            $rel_plugin_path = $this->sub_folder .trim(str_replace(site_url(),'', WP_PLUGIN_URL), '/');
        ...
        if ($this->opt('hide_other_wp_files') && $this->is_permalink()){
            $rel_content_path = $this->sub_folder . trim(str_replace(site_url(),'', WP_CONTENT_URL), '/');
            $rel_plugin_path = $this->sub_folder . trim(str_replace(site_url(),'', WP_PLUGIN_URL), '/');
          ...
          //      $style_path_reg = '|'.$rel_theme_path_with_theme.'/style\.css';
            ...
            $new_non_wp_rules[$rel_theme_path_with_theme.'/screenshot\.png|'.$this->sub_folder .'readme\.html|'.$this->sub_folder .'license\.txt|'.$rel_content_path.'/debug\.log'.$style_path_reg.'|'.$rel_include_path.'/$'] = 'nothing_404_404';   
        }
...
;
        ...
        if ($output)
            ...
            $output = '# You may need to re-configure the server whenever you change settings or activate a new theme or plugin.' . "\n\n" . '# If you use sub-directory for WP block you should add that directory before all of below pathes (e.g. rewrite ^/wordpress/lib/(.*) /wordpress/wp-includes/$1 or rewrite ^/wordpress/(.*).php$ /wordpress/nothing_404_404)'. "\n\n" . $output; 
        else
            $output=__('Nothing to add for current settings.', self::slug);      
        
        $html='';
        $desc = __( 'Add to Nginx config file to get all features of the plugin (/etc/nginx/nginx.conf or /etc/nginx/conf/nginx.conf). ', self::slug ) ;
        if (isset($_GET['nginx_config']) && $_GET['nginx_config'])  {
            
            $html= sprintf( '<strong> %s </strong><br/>', $desc );
            $html.= sprintf( '<textarea readonly="readonly" onclick="" rows="5" cols="55" class="regular-text %1$s" id="%2$s" name="%2$s" style="%4$s">%3$s</textarea>', 'nginx_config_class','nginx_config', esc_textarea($output), 'width:95% !important;height:400px !important' );
    ...
    function single_config(){
        ...
        $rel_plugin_path = $sub_install .trim(str_replace(site_url(),'', WP_PLUGIN_URL), '/');
        ...
        $rel_content_path = $sub_install . trim(str_replace(site_url(),'', WP_CONTENT_URL), '/');
        $rel_theme_path_no_template = str_replace('/'.get_stylesheet(), '', $rel_theme_path);
...
;
        ...
        if ($new_style_name)
            $output.='RewriteRule ^'.$new_theme_path. '/'.str_replace('.','\.', $new_style_name).' /'.$rel_theme_path.'/style.css [QSA,L]'."\n";
        ...
        if (!$output)
            ...
            $output="# If you use BulletProof security plugin first secure htaccess file using that and then add below lines to your htaccess file using FTP. (Add them before: RewriteCond %{REQUEST_FILENAME} !-f). Next you may want to change htaccess permission to read-only(e.g. 666)\n" . '# You may need to re-configure the server whenever you change settings or activate a new theme or plugin. '."\n\n" . $output;      
        
        $html='';
        $desc = __( 'In rare cases you need to configure it manually.', self::slug ) ;
        
        if (isset($_GET['single_config']) && $_GET['single_config'])  { 
            $html= sprintf( '<strong> %s </strong><br/>', $desc );
            $html.= sprintf( '<textarea readonly="readonly" onclick="" rows="5" cols="55" class="regular-text %1$s" id="%2$s" name="%2$s" style="%4$s">%3$s</textarea>', 'single_config_class','single_config', esc_textarea($output), 'width:95% !important;height:400px !important' );
...
;
        ...
        if (!$output)
            ...
            $output='# You may need to re-configure the server whenever you change settings or activate a new plugin.'."\n\n" . $output;      
        
        $html='';
        $desc = __( 'Add to .htaccess file to get all features of the plugin (after wp-admin or files section).', self::slug ) ;
        if (isset($_GET['multisite_config']) && $_GET['multisite_config'])  {
            
            $html= sprintf( '<strong> %s </strong><br/>', $desc );
            $html.= sprintf( '<textarea readonly="readonly" onclick="" rows="5" cols="55" class="regular-text %1$s" id="%2$s" name="%2$s" style="%4$s">%3$s</textarea>', 'multisite_config_class','multisite_config', esc_textarea($output), 'width:95% !important;height:400px !important' );
